...
class StickerPackLoader {
    ...
    private static final String TAG1 = "Authority";
    /**
    ...
    static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        ...
        Log.i(TAG, "fetchStickerPacks()" );
        Uri uri = StickerContentProvider.AUTHORITY_URI;
        String authority = StickerContentProvider.authority;
        ...
        if (cursor == null) {
            throw new IllegalStateException("could not fetch from content provider, " + authority);
        }
    ...
    private static ArrayList<StickerPack> fetchFromContentProvider(Cursor cursor) {
        ...
        Log.i(TAG, "fetchFromContentProvider()" );
    ...
    private static List<Sticker> fetchFromContentProviderForStickers(String identifier, ContentResolver contentResolver) {
        ...
        Log.i(TAG, "fetchFromContentProviderForStickers()" );
    ...
    private static Uri getStickerListUri(String identifier) {
        ...
        Log.i(TAG, "getStickerListUri()" );
        Log.i(TAG1, "StickerPackLoader, -getStickerListUri()" );


        String authority = StickerContentProvider.authority;
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(authority).appendPath(StickerContentProvider.STICKERS).appendPath(identifier).build();
    }

    static Uri getStickerAssetUri(String identifier, String stickerName) {
        Log.i(TAG, "getStickerAssetUri()" );
        Log.i(TAG1, "StickerPackLoader, -getStickerAssetUri()" );

        String authority = StickerContentProvider.authority;
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(authority).appendPath(StickerContentProvider.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build();
    }
}
